...
def run_task(task: str, mode: str = "real", state_dict: dict = None, test_mode: bool = False, forced_outcome: dict = None, task_id: str = None, user_id: str = "anonymous") -> dict:
    ...
    if _key_match:
        ...
        print(f"  🔑 [loop] KEY= pattern detected. Storing {_token}")
        try:
            from core.agent_mode.credential_vault import CredentialVault
            _vault = CredentialVault()
            _vault.store(_token, _key_value, "default")
            print(f"  ✅ [loop] Credential {_token} stored. Re-running: {_real_task[:40]}")
    ...
    try:
        ...
        if is_agent_mode_on("default"):
            print(f"  🤖 [agent_mode] Agent Creation Mode ON. Checking task: {task[:40]}")
            _agent_result = agent_handle(task, "default")
            ...
            print(f"  🤖 [agent_mode] No API needed, continuing normal flow.")
    except Exception as _ae:
        print(f"  ⚠️ [agent_mode] Check failed: {_ae}. Continuing normal flow.")
